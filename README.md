...
## Estado (14-sep-2026, v0.2b2 publicada)
...
- **Verificación**: 412 tests (`node --test tests/*.test.js`: motor, pipeline, servidor, sincronización, átomo), 52 tests e2e con Playwright (`npx playwright test`) a 375×812, 393×852 con zonas seguras y 430×932 (el servidor se simula con `page.route`, nunca se llama al real) con capturas en `docs/capturas/` (incluidas `docs/capturas/430x932/`), más una pasada adversarial (revisor final + adversarial gratis) con sus hallazgos arreglados o refutados con motivo (v0.1c en `ola-final-report.md`, v0.1d en `adversarial-v01d-triage.md`, ambos en `.superpowers/sdd/2026-09-12-one-v0.1c-mazo-vertical/`, carpeta git-ignored).
...
0. **Hecho en v0.2b2** (cliente conectado + Átomo, ver Estado). Siguientes, por orden: **v0.2a.2 protagonismo visual** (plan `docs/superpowers/plans/2026-09-14-one-v0.2a2-protagonismo-visual.md`, spec §8), **v0.2b1.1 cascada gratis robusta** (sin pago; añade Gemini gratis como proveedor), **amplitud del átomo** (nodo "Más…", 6 anillos, hilos de finanzas; pendiente de aprobar).
