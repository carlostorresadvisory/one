...
- **Verificación**: 486 tests (`node --test tests/*.test.js`: motor, pipeline, servidor, sincronización, átomo), 70 tests e2e con Playwright (`npx playwright test`) a 375×812, 393×852 con zonas seguras y 430×932 (el servidor se simula con `page.route`, nunca se llama al real) con capturas en `docs/capturas/` (incluidas `docs/capturas/430x932/`), más una pasada adversarial (revisor final + adversarial gratis) con sus hallazgos arreglados o refutados con motivo (v0.1c en `ola-final-report.md`, v0.1d en `adversarial-v01d-triage.md`, ambos en `.superpowers/sdd/2026-09-12-one-v0.1c-mazo-vertical/`, carpeta git-ignored).
